                                    ...
                                    if elite_level == None:
                                          ...
                                          if lower_level_name.__contains__(level):
                                                ...
                                                if category == None:
                                                      category = json_category
        if elite_level != None:
                print(f"\"game\":\"{elite_game}\",\"category_name\":\"{categoryName(game, category, jump)}\",\"level\":\"{elite_level}\",\"is_score\":{typebool}")
                smb_elite_data = supabase.rpc('get_chart_submissions', {"game":f"{elite_game}","category_name":f"{categoryName(game, category, jump)}","level":f"{elite_level}","is_score":typebool}).execute()
                if smb_elite_data.data == '[]':
                      color = 'blue'
                      smb_elite_data = supabase.rpc('get_chart_submissions', {"game":f"{elite_game}","category_name":f"{categoryName(game, category, jump)}","level":f"{elite_level}_(blue)","is_score":typebool}).execute()
                if (str(smb_elite_data.data).__contains__('\'medal\': \'platinum\'')):
                      for entry in smb_elite_data.data:
                            if entry["medal"] == "platinum":
                                  ...
                                  response = responseMessage(game, entry, response_level, color, type)
...
def levelName(game:str, level: str, color: str, jump: str):
      ...
      if level.__contains__('smb2 casual 1') or level.__contains__('smb2 casual 2') or level.__contains__('smb2 casual 3') or level.__contains__('smb2 casual 4'):
...
def responseMessage(game: str, entry, level:str, color:str, type:str):
      ...
      response:str = f"World record for {level} is {record} by **{entry["profile"]["username"]}** {entry["proof"]}"
      return(response)